...
- **SQLite** at `~/.config/krang/krang.db` for task/event storage
...
- **Dormant** — no tmux window, session ID saved for `--resume`
- **Completed/Failed** — terminal states

## Window Naming

- `K!<name>` — krang-managed Claude Code task windows
- `KF!<name>` — companion windows associated with a task (travel with the task on park/unpark)
...
- `internal/task/` — high-level lifecycle (create, park, wake, etc.) and reconciliation
...
Tasks are shut down via SIGINT to the Claude process (found via `pgrep -P <shell_pid>`), with a 5-second timeout before falling back to `tmux kill-window`.

## Hook Events

Krang listens for: `SessionStart`, `UserPromptSubmit`, `Stop`, `PermissionRequest`, `TaskCompleted`, `StopFailure`, `Notification`, `SessionEnd`. Events are matched to tasks by `session_id`. Resumed sessions get adopted via cwd matching on `SessionStart`.
